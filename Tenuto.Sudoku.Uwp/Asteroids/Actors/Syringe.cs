...
namespace Tenuto.Asteroids.Actors
{
    public class Syringe
    {
        ...
        private readonly Vector2[] _syringeBody = new Vector2[15]; // syringe body
        ...
        public float Size => 8;
        public Vector2 NeedleTop => TransformPoint(0, -30);
        ...
        public void Draw(CanvasDrawingSession ds)
        {
            ...
            {
                CalcSyringeBody();
                var geom = CanvasGeometry.CreatePolygon(ds, _syringeBody);
                if (IsGhost && !_isStatus)
                    ds.DrawGeometry(geom, Colors.Silver, 1, new CanvasStrokeStyle { DashStyle = CanvasDashStyle.DashDot, MiterLimit = 1 });
                else
                    ds.FillGeometry(geom, Colors.Silver);

                var pneedle1 = TransformPoint(0, -20);
                var pneedle2 = TransformPoint(0, -40);

                ds.DrawLine(pneedle1, pneedle2, Colors.Silver, 2);
        ...
        private Vector2 TransformPoint(float x, float y)
        {
            var cosr = (float)Math.Cos(Rotation);
            var sinr = (float)Math.Sin(Rotation);
            var xx = x * cosr - y * sinr;
            var yy = x * sinr + y * cosr;
            return new Vector2(_position.X + xx, _position.Y + yy);
        }

        private void CalcSyringeBody()
        {
            var i = 0;
            _syringeBody[i++] = TransformPoint(-5, -20);  // left  top  
            _syringeBody[i++] = TransformPoint(-5, 16);   // left  bottom 

            _syringeBody[i++] = TransformPoint(-8, 16);   //-- 
            _syringeBody[i++] = TransformPoint(-8, 17);   //-- 
            _syringeBody[i++] = TransformPoint(-3, 17);   //-- 

            _syringeBody[i++] = TransformPoint(-3, 19);   //-- 
            _syringeBody[i++] = TransformPoint(-5, 19);   //-- 

            _syringeBody[i++] = TransformPoint(-5, 20);   //-- 
            ...
            _syringeBody[i++] = TransformPoint(+5, 16);   // right bottom 
            _syringeBody[i++] = TransformPoint(+5, -20);  // right top
        }
